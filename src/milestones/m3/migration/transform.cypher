-- M2 → M3 transform
-- SpecRun gains reasoning_step_count and evaluation_depth; backfill M2 rows with neutral values.
...
-- REASONING_ADDS excluded — new table; no M2 rows to carry forward.
...
-- SpecRun: M2 columns + backfill new M3 columns
-- reasoning_step_count=0: M2 used COSTAR only — no CoT steps existed
-- evaluation_depth='unknown': CoT not present in M2; not applicable to those runs
COPY (
  MATCH (n:SpecRun)
