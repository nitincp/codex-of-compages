# [M5-origin]
"""
SpecAdvisorAgent — M5: COSTAR (Structure) + ChainOfThought (Reasoning)
+ ConstitutionalAI (Verification) — M4 chain unchanged.

After the LLM call, build_crispe_prompt() fills a CRISPEPrompt from the structured
output and appends it as specialist_crispe_prompt. This is the meta-prompting moment:
the Advisor's output becomes the Spec Specialist's system prompt.

Composition chain (M5): COSTARPrompt → ChainOfThought → ConstitutionalAI (identical to M4)
Meta-prompt step: build_crispe_prompt(output) → specialist_crispe_prompt appended post-LLM

Frozen for the M5 milestone — do not import from src.agents.
"""
...
from src.milestones.m5.frameworks.chain_of_thought import ChainOfThought
from src.milestones.m5.frameworks.composed import ComposedPrompt
from src.milestones.m5.frameworks.constitutional_ai import ConstitutionalAI
from src.milestones.m5.frameworks.costar import COSTARPrompt
from src.milestones.m5.frameworks.crispe import CRISPEPrompt
...
def build_crispe_prompt(output: SpecAdvisorOutput) -> str:
    """Build a CRISPE prompt from a SpecAdvisorOutput for injection into the Spec Specialist."""
    crispe = CRISPEPrompt(
        capacity=f"{output.selected_lang} specialist",
        role=(
            "You are the Spec Specialist in the Faber prompt engineering council. "
            "Your task is to produce a formal specification in the language specified by "
            "the Capacity field, for the project brief given by the user."
        ),
        insight=output.justification,
        statement=(
            f"Generate a {output.layer}-layer {output.selected_lang} specification for the "
            "given project brief. The specification must be syntactically correct, complete, "
            "and cover the primary concerns identified in the Insight above."
        ),
    )
    return crispe.build()
...
class SpecAdvisorAgent(BaseAgent[SpecAdvisorOutput]):
    """Selects the optimal formal spec language and generates a CRISPE meta-prompt."""
    ...
    def run(self, project_brief: str) -> SpecAdvisorOutput:
        """Select a spec language and generate the CRISPE meta-prompt for the Spec Specialist."""
        ...
        base = SpecAdvisorOutput.model_validate(tool_block.input)

        crispe_prompt = build_crispe_prompt(base)
        return base.model_copy(update={"specialist_crispe_prompt": crispe_prompt})
